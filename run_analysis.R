...
meanStdMeasure <- grep("mean\\(\\)|std\\(\\)", features[, 2])
dataJoin <- dataJoin[, meanStdMeasure]


## Use descriptive activity names to name the activities in the dataset
colnames(dataJoin) <- gsub("\\(\\)", "", features[meanStdMeasure, 2]) # remove parentheses
...
# Merge Data
cleanedData <- cbind(subJoin, labelJoin, dataJoin)


# write out the 1st dataset as tab delimited text file
write.table(cleanedData, "TidyDataSet.txt", sep='\t', quote=FALSE, row.names=FALSE) 
...
result <- matrix(NA, nrow=subLength*activityLen, ncol=columnLen) 
